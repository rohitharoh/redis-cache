...
//go	cache.NewRedisCache(viper.GetString("redis.addr"), 0, system.REDIS_DEFAULT_EXPIRATION_TIME).PSubPub(key)
...
func PublishMessage () {
	...
	body := "Cache Empty !!... Fetching from DB."

	conn := system.TbAppContext.RabbitMQConn
	...
	err = channel.Publish("", "first.queue", false, false, amqp.Publishing{
	...
	log.Info("Published the message")
